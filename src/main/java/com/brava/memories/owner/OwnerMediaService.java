...
public class OwnerMediaService {
 ...
 @Transactional(readOnly=true) public MediaDtos.Page list(UUID eventId,String principal,boolean admin,int page,int size){Event e=events.requireOwned(eventId,principal,admin);Page<Media> r=media.findByEventIdOrderByCreatedAtDesc(e.getId(),PageRequest.of(Math.max(0,page),Math.min(Math.max(1,size),100)));return new MediaDtos.Page(r.getContent().stream().map(this::item).toList(),r.getNumber(),r.getSize(),r.getTotalElements(),r.getTotalPages());}

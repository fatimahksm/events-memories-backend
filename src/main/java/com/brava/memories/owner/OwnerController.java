...
public class OwnerController {
 ...
 @GetMapping("/events/{eventId}/media") public MediaDtos.Page media(@PathVariable UUID eventId,@AuthenticationPrincipal Jwt jwt,@RequestParam(defaultValue="0") int page,@RequestParam(defaultValue="50") int size,@RequestParam(required=false) MediaVisibility visibility,@RequestParam(required=false) java.time.Instant from,@RequestParam(required=false) java.time.Instant to){return media.list(eventId,jwt.getSubject(),admin(jwt),page,size,visibility,from,to);}

...
import com.brava.memories.event.EventRepository;
...
public class AuthService {
 ...
 private LoginResult issueToken(AppUser u,UUID scopedEventId){
   Instant now=Instant.now();
   JwtClaimsSet.Builder builder=JwtClaimsSet.builder().issuer("event-memories").issuedAt(now).expiresAt(now.plus(props.security().jwtTtl())).subject(u.getId().toString()).claim("email",u.getEmail()).claim("name",u.getDisplayName()).claim("roles", List.of(u.getRole().name()));
   if(scopedEventId!=null) builder.claim("eventId",scopedEventId.toString());
   ...
   String scoped=scopedEventId!=null?scopedEventId.toString():null;
   return new LoginResult(token,new AuthDtos.MeResponse(u.getId().toString(),u.getEmail(),u.getDisplayName(),u.getRole().name(),scoped));
 }
 ...
 public LoginResult eventAccess(String token){
   Event e=events.findByAccessToken(token).orElseThrow(()->new AppException("INVALID_ACCESS_LINK","This access link is invalid or has expired",HttpStatus.UNAUTHORIZED));
   AppUser owner=e.getOwner();
   if(!owner.isEnabled()) throw new AppException("ACCOUNT_DISABLED","This account has been disabled",HttpStatus.UNAUTHORIZED);
   return issueToken(owner,e.getId());
 }
 public AuthDtos.MeResponse me(String id,String scopedEventId){AppUser u=users.findById(java.util.UUID.fromString(id)).orElseThrow(()->new AppException("USER_NOT_FOUND","User not found",HttpStatus.NOT_FOUND));return new AuthDtos.MeResponse(u.getId().toString(),u.getEmail(),u.getDisplayName(),u.getRole().name(),scopedEventId);}
 @Transactional

    ...
    @Bean SecurityFilterChain filterChain(HttpSecurity http, BearerTokenResolver resolver) throws Exception {
        http.csrf(csrf -> csrf.disable())
            ...
            .authorizeHttpRequests(auth -> auth
                .requestMatchers("/actuator/health", "/api/auth/login", "/api/auth/register", "/api/auth/owner-access", "/api/auth/event-access", "/api/auth/forgot-password", "/api/auth/reset-password", "/api/public/**").permitAll()

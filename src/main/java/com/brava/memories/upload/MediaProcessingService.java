...
public class MediaProcessingService {
 ...
 private final MediaRepository mediaRepo; private final ObjectStorage storage; private final UploadValidationService validation; private final MalwareScanner scanner; private final ThumbnailService thumbnails; private final VideoTranscodingService video; private final TransactionTemplate tx;
 public MediaProcessingService(MediaRepository mediaRepo,ObjectStorage storage,UploadValidationService validation,MalwareScanner scanner,ThumbnailService thumbnails,VideoTranscodingService video,PlatformTransactionManager tm){this.mediaRepo=mediaRepo;this.storage=storage;this.validation=validation;this.scanner=scanner;this.thumbnails=thumbnails;this.video=video;this.tx=new TransactionTemplate(tm);}
 ...
 private void process(UUID id,boolean staleRecovery){
   ...
   try{
     ...
     if(!storage.exists(m.storageKey())){if(storage.exists(finalKey)){Derivatives d=deriveAssets(m,finalKey);ready(id,finalKey,d.thumbnailKey(),d.renditionKey());return;}fail(id,"OBJECT_NOT_FOUND");return;}
     ...
     Derivatives d=deriveAssets(m,finalKey);
     ready(id,finalKey,d.thumbnailKey(),d.renditionKey());
 ...
 private Derivatives deriveAssets(MediaSnapshot m,String finalKey){
   if(m.mediaType()==MediaType.IMAGE)return new Derivatives(generateThumbnail(m,finalKey),null);
   if(m.mediaType()==MediaType.VIDEO)return new Derivatives(generatePoster(m,finalKey),generateRendition(m,finalKey));
 ...
 private record Derivatives(String thumbnailKey,String renditionKey){}
}
